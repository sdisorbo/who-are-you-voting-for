...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    // Fetch election
    const [election] = await sql`SELECT id FROM elections WHERE slug = ${slug} AND type = 'current'`
    if (!election) return NextResponse.json({ error: 'Election not found' }, { status: 404 })

    const electionId = election.id

    // Hash IP — never store or log raw IP
    ...
    console.error('POST /api/elections/[slug]/vote error:', err)
